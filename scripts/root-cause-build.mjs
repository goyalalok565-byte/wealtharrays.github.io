...
for(const obj of objects){const id=obj.match(/\bid:\s*["']([^"']+)["']/)?.[1];if(!id)throw new Error('Calculator definition has no id');fs.writeFileSync(path.join(defDir,`${id}.js`),`/* Generated from calculators.js — do not edit directly. */\nwindow.CALCULATORS=window.CALCULATORS||[];window.CALCULATORS.push(${obj});\n`,'utf8');}
...
html=html.replace(/<link\s+rel=["'](?:shortcut )?icon["'][^>]*>/gi,'');html=html.replace('</head>','<link rel="icon" href="/favicon-v3.svg?v=20260917" type="image/svg+xml" sizes="128x128"><link rel="shortcut icon" href="/favicon-v3.svg?v=20260917" type="image/svg+xml"><link rel="apple-touch-icon" href="/icon-192.png"></head>');
...
const unique=[...new Set(routes)];const today=new Date().toISOString().slice(0,10);const sitemap=['<?xml version="1.0" encoding="UTF-8"?>','<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">',...unique.map(u=>`  <url><loc>https://wealtharrays.com${u}</loc><lastmod>${today}</lastmod></url>`),'</urlset>',''].join('\n');fs.writeFileSync(path.join(root,'sitemap.xml'),sitemap,'utf8');console.log(`Root-cause build complete: canonical site search, calculator search, current favicon, 20 split definitions, absolute styles, CLS reservation, deterministic sitemap, ${allHtml.length} HTML files normalized.`);
