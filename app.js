...
function renderHome() {
  ...
  const categoryChips = CATEGORIES.map((cat) => {
    ...
    if (!count) return "";
    return `<a class="chip" href="#/category/${cat.id}">${cat.label} <span class="chip-count">${count}</span></a>`;
  }).join("");

  APP.innerHTML = `
    <section class="ledger-hero">
      <h1>Financial calculators, kept honest.</h1>
      <p>No advisor referrals, no lead-selling, no signup wall. Pick a calculator below — every result is computed instantly in your browser, in the currency you choose.</p>
    ...
    <section class="ledger-index">
      <div class="ledger-index-heading" id="index-heading">INDEX — ${CALCULATORS.length} CALCULATORS</div>
