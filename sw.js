const VERSION='wealtharrays-pwa-4';
...
self.addEventListener('fetch',event=>{
  ...
  if(request.mode==='navigate'){
    event.respondWith(fetch(request,{cache:'no-store'}).then(response=>{cachePut(RUNTIME_CACHE,request,response);return response;}).catch(()=>caches.match(request).then(hit=>hit||caches.match('/offline.html'))));
    return;
  }
  ...
  if(asset){
    event.respondWith(caches.match(request).then(hit=>hit||fetch(request).then(response=>{cachePut(RUNTIME_CACHE,request,response);return response;}).catch(()=>caches.match('/offline.html'))));
    return;
  }
  event.respondWith(fetch(request).then(response=>{cachePut(RUNTIME_CACHE,request,response);return response;}).catch(()=>caches.match(request)));
});
