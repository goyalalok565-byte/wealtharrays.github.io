...
if(cur){cur.innerHTML=C.map(x=>`<option value="${x[0]}">${x[0]} · ${x[1]} · ${x[2]}</option>`).join('');const savedCurrency=localStorage.waCurrency||'INR';cur.value=C.some(x=>x[0]===savedCurrency)?savedCurrency:'INR';document.documentElement.dataset.currency=cur.value;cur.onchange=()=>{localStorage.waCurrency=cur.value;document.documentElement.dataset.currency=cur.value;window.waTrack?.('currency_change',{currency:cur.value});window.dispatchEvent(new Event('wa-currency'))}}
