...
(function () {
  ...
  const RELATED = {
    ...
    salary:[['salary-to-hourly-calculator.html','Salary to Hourly'],['overtime-pay-calculator.html','Overtime Pay'],['freelance-rate-calculator.html','Freelance Rate'],['income-tax-scenario-calculator.html','Income Tax Scenario Calculator']],
    business:[['profit-margin-calculator.html','Profit Margin'],['roi-calculator.html','ROI Calculator'],['freelance-rate-calculator.html','Freelance Rate'],['income-tax-scenario-calculator.html','Income Tax Scenario Calculator']]
  };
  ...
  function calculatorSearch() {
    ...
    const aliases = {sip:'systematic investment plan monthly mutual fund',emi:'mortgage home loan monthly payment',fd:'fixed deposit banking',rd:'recurring deposit monthly banking',roi:'return investment profit',cagr:'compound annual growth return',tax:'income tax scenario',salary:'hourly wage income',loan:'mortgage personal car debt',retirement:'pension financial independence',inflation:'purchasing power future prices',lumpsum:'lump sum investment one time','net worth':'assets liabilities wealth',networth:'assets liabilities wealth','compound interest':'compounding growth','simple interest':'flat interest','profit margin':'business revenue cost margin','car loan':'vehicle loan emi','personal loan':'loan repayment',freelance:'freelance rate hourly pricing',overtime:'overtime pay salary'};
    function render() {
  ...
  function showInstallBanner() {
    if (!installEvent || (window.matchMedia && window.matchMedia('(display-mode: standalone)').matches) || window.navigator.standalone === true) return;
    const dismissed = Number(get('waInstallDismissedV3','0')); if (dismissed && Date.now() - dismissed < 7*24*60*60*1000) return;
    if (document.getElementById('wa-install')) return;
    installStyles();
    const box = document.createElement('aside'); box.id = 'wa-install'; box.setAttribute('role','status');
    box.innerHTML = '<div><strong>Install Wealth Arrays</strong><p>Use Wealth Arrays like an app from your home screen.</p></div><div class="wa-install-actions"><button type="button" data-wa-install-action="close">Not now</button><button type="button" data-wa-install-action="install">Install app</button></div>';
    document.body.appendChild(box);
    box.addEventListener('click', async function(event){
      const button = event.target.closest('[data-wa-install-action]'); if (!button) return;
      if (button.dataset.waInstallAction === 'close') { set('waInstallDismissedV3',String(Date.now())); box.remove(); return; }
      if (!installEvent) { box.remove(); return; }
      const promptEvent = installEvent; installEvent = null; window.__WA_INSTALL_PROMPT__ = null;
      try { promptEvent.prompt(); await promptEvent.userChoice; } catch (error) {}
      box.remove();
    });
  }
  ...
  window.addEventListener('appinstalled', function(){ installEvent = null; window.__WA_INSTALL_PROMPT__ = null; const box = document.getElementById('wa-install'); if (box) box.remove(); set('waInstallDismissedV3',String(Date.now())); });
