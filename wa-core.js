...
(function () {
  ...
  function setupTheme() {
    applyTheme(get('waTheme', document.documentElement.dataset.theme === 'dark' ? 'dark' : 'light'));
    if (document.documentElement.dataset.waThemeOwner === '1') return;
    document.documentElement.dataset.waThemeOwner = '1';
  ...
  function applyCurrency() {
    ...
    if (select.dataset.waCurrencyOwner === '1') return;
  ...
  function calculatorSearch() {
    ...
    function render() {
      ...
      const terms = (query + ' ' + expanded).split(/\s+/).filter(Boolean);
  ...
  function analytics() {
    ...
    window.waTrack = function (name, params) { try { if (get('waAnalyticsConsent','') === 'granted') window.gtag('event', name, params || {}); } catch (error) {} };
  ...
  function consentDialog() {
    if (get('waAnalyticsConsent','') || document.getElementById('wa-consent')) return;
    ...
    box.addEventListener('click', (event) => {
      const button = event.target.closest('[data-wa-consent]');
      if (!button) return;
      const granted = button.dataset.waConsent === 'accept';
      set('waAnalyticsConsent', granted ? 'granted' : 'denied');
  ...
  function registerServiceWorker() {
    ...
    const start = () => navigator.serviceWorker.register('/sw.js',{scope:'/'}).catch((error) => console.warn('Wealth Arrays service worker registration failed:', error));
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', start, { once:true }); else start();
  }

  function styles() {
    if (document.getElementById('wa-core-css')) return;
    const style = document.createElement('style'); style.id = 'wa-core-css';
  ...
  function init() {
    styles(); setupTheme(); applyCurrency(); applyCategories(); calculatorSearch(); addRelatedAndTrust(); renderFooter(); analytics(); consentDialog(); registerServiceWorker();
  }
