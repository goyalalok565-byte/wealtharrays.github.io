...
(function(){
  ...
  async function makePdf(calc,values,results){
    ...
    const line=(s,size=9,bold=false)=>{const words=text(s).split(/\s+/);let cur='',lines=[];words.forEach(w=>{const next=cur?cur+' '+w:w;if(next.length>Math.max(30,Math.floor(CW/(size*.52)))&&cur){lines.push(cur);cur=w}else cur=next});if(cur||!lines.length)lines.push(cur);lines.forEach(v=>{ensure(size+8);cmd.push('BT /F'+(bold?2:1)+' '+size+' Tf '+M+' '+y+' Td ('+esc(v)+') Tj ET');y-=size+8});y-=2};
    ...
    const objects=[null,enc('<< /Type /Catalog /Pages 2 0 R >>'),null],f1=objects.length;objects.push(enc('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>')),f2=objects.length;objects.push(enc('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold >>'));
    let imgObj=null;
