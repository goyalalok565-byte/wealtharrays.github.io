...
(function(){
  ...
  async function makePdf(calc,values,results){
    ...
    const line=(s,size=9,bold=false)=>{const words=text(s).split(/\s+/);let cur='',lines=[];words.forEach(w=>{const next=cur?cur+' '+w:w;if(next.length>Math.max(30,Math.floor(CW/(size*.52)))&&cur){lines.push(cur);cur=w}else cur=next});if(cur||!lines.length)lines.push(cur);lines.forEach(v=>{ensure(size+8);cmd.push('0 0 0 rg BT /F'+(bold?2:1)+' '+size+' Tf '+M+' '+y+' Td ('+esc(v)+') Tj ET');y-=size+8});y-=2};
    const center=(s,size=9,bold=false)=>{const v=text(s),w=v.length*size*.5;ensure(size+8);cmd.push('0 0 0 rg BT /F'+(bold?2:1)+' '+size+' Tf '+Math.max(M,(W-w)/2)+' '+y+' Td ('+esc(v)+') Tj ET');y-=size+8};
    const table=(title,rows)=>{ensure(38);line(title,15,true);y-=2;rows.forEach((r,i)=>{ensure(25);if(i%2===0)cmd.push('0.97 0.98 0.99 rg '+M+' '+(y-19)+' '+CW+' 23 re f');cmd.push('0.86 0.88 0.91 RG 0.5 w '+M+' '+(y-19)+' '+CW+' 23 re S');const a=text(r[0]).slice(0,68),b=text(r[1]).slice(0,48),rw=Math.min(220,b.length*4.5);cmd.push('0 0 0 rg BT /F1 9 Tf '+(M+8)+' '+(y-13)+' Td ('+esc(a)+') Tj ET');cmd.push('0 0 0 rg BT /F2 9 Tf '+(W-M-8-rw)+' '+(y-13)+' Td ('+esc(b)+') Tj ET');y-=23});y-=9};
    const pie=(parts)=>{ensure(245);line('Result breakdown',15,true);line('The same genuine components shown by the calculator chart.',9,false);y-=5;const total=parts.reduce((s,p)=>s+p.value,0),max=Math.max(...parts.map(p=>p.value),1),barW=285,colors=['0.15 0.39 0.92','0.08 0.72 0.65','0.49 0.24 0.93','0.96 0.62 0.04'];parts.forEach((p,i)=>{ensure(38);const pct=p.value/total*100,label=text(p.label).slice(0,38),value=fmt({value:p.value,format:'currency'});cmd.push('0.92 0.93 0.95 rg '+(M+145)+' '+(y-9)+' '+barW+' 12 re f');cmd.push(colors[i%colors.length]+' rg '+(M+145)+' '+(y-9)+' '+Math.max(4,barW*p.value/max).toFixed(1)+' 12 re f');cmd.push('0 0 0 rg BT /F1 8 Tf '+M+' '+(y-7)+' Td ('+esc(label)+') Tj ET');cmd.push('0 0 0 rg BT /F2 8 Tf '+(M+440)+' '+(y-7)+' Td ('+esc(value+' - '+pct.toFixed(pct<10?1:0)+'%')+') Tj ET');y-=29});y-=7};
    const bars=(rows)=>{ensure(260);line('Calculation summary',15,true);line('The bars use the actual calculated values.',9,false);y-=5;const numeric=rows.filter(r=>Number.isFinite(Number(r.value))&&Number(r.value)>=0).slice(0,6),max=Math.max(...numeric.map(r=>Number(r.value)),1);numeric.forEach(r=>{ensure(30);const label=text(r.label).slice(0,30),v=Number(r.value),bw=250*v/max;cmd.push('0.92 0.93 0.95 rg '+(M+145)+' '+(y-8)+' 250 10 re f');cmd.push('0.15 0.39 0.92 rg '+(M+145)+' '+(y-8)+' '+Math.max(4,bw).toFixed(1)+' 10 re f');cmd.push('0 0 0 rg BT /F1 8 Tf '+M+' '+(y-6)+' Td ('+esc(label)+') Tj ET');cmd.push('0 0 0 rg BT /F2 8 Tf '+(M+405)+' '+(y-6)+' Td ('+esc(fmt(r))+') Tj ET');y-=27});y-=8};

    // Real Wealth Arrays logo from the same icon used by the site.
    if(logo){ensure(58);const iw=46,ih=46,ix=M,iy=y-ih+8;cmd.push('q '+iw+' 0 0 '+ih+' '+ix+' '+iy+' cm /Im1 Do Q');cmd.push('BT /F2 18 Tf '+(ix+58)+' '+(y-22)+' Td (WEALTH ARRAYS) Tj ET');y-=58}
    else {center('WEALTH ARRAYS',18,true);y-=2}
    center('CALCULATION REPORT',8,true);
    cmd.push('0.86 0.88 0.91 RG 0.7 w '+M+' '+(y-0.5)+' '+CW+' 1 re S');y-=16;
    ...
    const f1=objects.length;
    objects.push(enc('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>'));
    const f2=objects.length;
    objects.push(enc('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold >>'));
    let imgObj=null;
