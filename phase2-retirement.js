...
(function () {
  ...
  function render(calc,anchor,el){
    const fields=calc.fields||[],values={};
    fields.forEach(f=>{const q=`#${CSS.escape(f.id)},[name="${CSS.escape(f.id)}"]`,node=document.getElementById('calc-widget')?.querySelector(q),x=Number(node?.value);values[f.id]=Number.isFinite(x)?x:Number(f.default);});
    ...
    el.innerHTML=`<div class="wa2-kicker">RETIREMENT DECISION VIEW</div><h2>Target, gap and sensitivity</h2><p class="wa2-muted">The headline target is only the starting point. This view separates the portfolio target from what you already have and shows how strongly the result depends on the withdrawal-rate assumption.</p><div class="wa2-retirement-grid"><div><span>Estimated target</span><strong>${esc(fmt(target))}</strong></div><div><span>Still needed</span><strong>${esc(fmt(shortfall))}</strong></div><div><span>Current progress</span><strong>${progress.toFixed(1)}%</strong></div></div><div class="wa2-table-wrap"><table class="wa2-table"><thead><tr><th>Withdrawal rate</th><th>Required target</th><th>Change vs current target</th></tr></thead><tbody>${scenarios.map(x=>`<tr class="${x.rate===rate?'is-base':''}"><td>${x.rate.toFixed(1)}%</td><td>${esc(fmt(x.target))}</td><td>${x.rate===rate?'Base':((x.target-target)/target*100).toFixed(1)+'%'}</td></tr>`).join('')}</tbody></table></div><p class="wa2-foot">A lower withdrawal rate produces a larger target. Historical withdrawal research is not a guarantee of future portfolio survival; taxes, fees, asset allocation, retirement length and spending changes can materially alter the outcome.</p>`;
    return true;
  }
  function mount(){
    const slug=location.pathname.split('/').pop().replace(/\.html$/,''),id=window.CALCULATORS?.find(c=>c.slug===slug||c.id===new URLSearchParams(location.search).get('calc'))?.id;
    if(id!=='freedom-milestone'||document.querySelector('[data-wa-retirement-intelligence]'))return;
    const calc=window.CALCULATORS.find(c=>c.id===id),anchor=document.querySelector('.tool-article')||document.getElementById('calc-widget')?.parentElement;if(!anchor)return;
    const el=document.createElement('section');el.className='wa2-panel wa2-retirement';el.dataset.waRetirementIntelligence='true';
    if(!render(calc,anchor,el))return;anchor.parentNode.insertBefore(el,anchor);
    let timer=0;const refresh=()=>{clearTimeout(timer);timer=setTimeout(()=>render(calc,anchor,el),100);};document.addEventListener('input',e=>{if(e.target.closest('#calc-widget'))refresh();},true);document.addEventListener('change',e=>{if(e.target.closest('#calc-widget'))refresh();},true);window.addEventListener('wa-currency',refresh);
  }
