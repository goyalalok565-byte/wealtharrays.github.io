...
const scriptNames = html => [...html.matchAll(/<script\s+src=["']([^"']+)["'][^>]*><\/script>/gi)].map(m => path.basename(m[1].split('?', 1)[0]));
const sourceFiles = [];
for (const dir of ['.', 'articles', 'p', '.github/workflows']) {
  const fullDir = path.join(root, dir); if (!fs.existsSync(fullDir)) continue;
  const walk = current => { for (const entry of fs.readdirSync(current, {withFileTypes:true})) { const full=path.join(current,entry.name); if(entry.isDirectory()) walk(full); else if(/\.(html|css|js|yml|yaml)$/i.test(entry.name)) sourceFiles.push(full); } };
  walk(fullDir);
}
...
for (const {file,text} of contents) if (clippingPattern.test(text)) throw new Error(`Recurring search clipping rule detected in ${file}`);
...
for (const {file,text} of contents) if (scriptNames(text).includes(forbiddenRuntime)) throw new Error(`Retired runtime reference detected in ${file}`);
...
for (const page of calculatorPages) {
  const names = scriptNames(fs.readFileSync(path.join(root,page),'utf8')).filter(name => name === 'wa-calculator-runtime.js');
  if (names.length !== 1) throw new Error(`${page}: expected exactly one canonical calculator runtime, found ${names.length}`);
}

if (!fs.existsSync(path.join(root,'wa-site-runtime.js'))) throw new Error('Canonical wa-site-runtime.js bundle is missing');
const htmlFiles=[]; const walkHtml=dir=>{ for(const entry of fs.readdirSync(dir,{withFileTypes:true})){ const full=path.join(dir,entry.name); if(entry.isDirectory()){if(!['.git','node_modules'].includes(entry.name)) walkHtml(full);} else if(/\.html$/i.test(entry.name)) htmlFiles.push(full); } }; walkHtml(root);
for (const full of htmlFiles) {
  const rel=path.relative(root,full).replaceAll(path.sep,'/'); if(calculatorPages.includes(rel)||rel==='404.html') continue;
  const names=scriptNames(fs.readFileSync(full,'utf8')); const hasSharedSource=siteSources.some(base=>names.includes(base)); const bundleCount=names.filter(name=>name==='wa-site-runtime.js').length;
  if(hasSharedSource||bundleCount){ if(bundleCount!==1) throw new Error(`${rel}: expected exactly one canonical site runtime, found ${bundleCount}`); for(const base of siteSources) if(names.includes(base)) throw new Error(`${rel}: direct shared module ${base} bypasses wa-site-runtime.js`); }
}
