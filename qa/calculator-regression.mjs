...
const expected = [
  'sip','compound-interest','mortgage','roi','simple-interest','freedom-milestone',
...
for (const id of expected) assert(actualIds.includes(id), `Required calculator id missing: ${id}`);
assert(new Set(actualIds).size === actualIds.length, 'Calculator IDs must be unique');
