...
(function(){
  ...
  function removeRetiredCards(){
    document.querySelectorAll('a,button,[data-search]').forEach(function(node){
      ...
      if(retired && node.closest('.home-tool-card,.tool-card,.ledger-row,.cat-card,.tools-grid,.home-tool-grid')){
        var card=node.closest('.home-tool-card,.tool-card,.ledger-row,.cat-card')||node;
        card.remove();
      }
    });
  }
