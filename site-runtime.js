...
(function(){
  ...
  function inferCategory(el){
    const text=((el.dataset.search||'')+' '+(el.querySelector('.tool-num,.home-tool-card small')?.textContent||'')+' '+el.textContent).toLowerCase();
  ...
  function cardAccents(){
    ...
    if(!document.getElementById('wa-universal-card-css')){
      const s=document.createElement('style');s.id='wa-universal-card-css';s.textContent='.tool-card[data-wa-category],.home-tool-card[data-wa-category],.ledger-row[data-wa-category]{border:1px solid color-mix(in srgb,var(--wa-card-accent) 24%,var(--line))!important;box-shadow:none!important}.tool-card[data-wa-category]:hover,.home-tool-card[data-wa-category]:hover,.ledger-row[data-wa-category]:hover{border-color:color-mix(in srgb,var(--wa-card-accent) 42%,var(--line))!important}';document.head.appendChild(s);
    }
  }
  function footer(){
    ...
    if(!document.getElementById('wa-universal-footer-css')){
      const s=document.createElement('style');s.id='wa-universal-footer-css';s.textContent='.footer-mark{display:inline-flex!important;align-items:center;gap:10px;font-weight:850;font-size:20px;line-height:1.1;letter-spacing:-.04em}.wa-footer-logo{width:30px;height:30px;display:block;flex:0 0 30px;object-fit:contain}.footer-mark sup{font-size:8px;align-self:flex-start;margin-top:1px}@media(max-width:600px){.footer-mark{font-size:19px}.wa-footer-logo{width:28px;height:28px;flex-basis:28px}}';document.head.appendChild(s);
    }
  }
  function search(){
    ...
    const render=()=>{
      ...
      box.innerHTML=matches.length?matches.map(({card})=>{const title=card.querySelector('b,h2')?.textContent?.trim()||card.textContent.trim().split('\n')[0];const desc=card.querySelector('p,small')?.textContent?.trim()||'';return'<a class="wa-search-suggestion" href="'+card.getAttribute('href')+'"><span><strong>'+title+'</strong><small>'+desc+'</small></span><b>→</b></a>'}).join(''):'<div class="tool-search-empty">No exact match. Try SIP, EMI, FD, RD, loan, tax, salary or ROI.</div>';
      box.hidden=false;
    };
