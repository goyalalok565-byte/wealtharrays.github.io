...
function waOpenPrintReport(calc,values,results){
 ...
 const total=numeric.reduce((s,r)=>s+r.value,0)||1;
 const bars=numeric.slice(0,6).map((r,i)=>{const pct=Math.max(4,Math.round(r.value/total*100));return `<div class="bar"><span>${esc(r.label)}</span><div><i style="width:${pct}%"></i></div><b>${pct}%</b></div>`;}).join('');
 const chart=bars?`<section class="chartbox"><h2>Result breakdown</h2><p>A visual share of the main calculated values.</p>${bars}</section>`:'';
 const html=`<!doctype html><html><head><meta charset="utf-8"><title>${esc(calc.title)} Report</title><style>body{font-family:Arial,sans-serif;max-width:760px;margin:36px auto;padding:0 24px;color:#101828}h1{font-size:30px;margin:0 0 6px}h2{font-size:17px;margin:28px 0 8px}p{color:#667085}table{width:100%;border-collapse:collapse;margin:10px 0 24px;border:1px solid #e4e7ec;border-radius:10px;overflow:hidden}td{padding:11px;border-bottom:1px solid #eaecf0}td:last-child{text-align:right;font-weight:700}.brand{color:#667085;font-size:12px;letter-spacing:.08em;font-weight:700}.chartbox{padding:18px;border:1px solid #e4e7ec;border-radius:14px;margin:24px 0}.chartbox h2{margin:0 0 4px}.bar{display:grid;grid-template-columns:140px 1fr 46px;gap:10px;align-items:center;margin:12px 0;font-size:12px}.bar>div{height:12px;background:#eef2f6;border-radius:999px;overflow:hidden}.bar i{display:block;height:100%;background:#2563eb;border-radius:999px}.bar b{text-align:right}@media print{body{margin:18px auto;padding:0}.no-print{display:none}}</style></head><body><div class="brand">WEALTH ARRAYS · CALCULATION REPORT</div><h1>${esc(calc.title)}</h1><p>Generated from the assumptions you entered.</p><h2>Your inputs</h2><table>${inputs}</table><h2>Your calculated results</h2><table>${rows}</table>${chart}<p>Educational estimate only. Not financial, tax, legal or investment advice.</p><button class="no-print" onclick="window.print()">Print / Save as PDF</button></body></html>`;
 const win=window.open('','_blank','noopener,noreferrer');
 if(!win){alert('Your browser blocked the report window. Please allow pop-ups for this site and try again.');return;}
 win.document.open();win.document.write(html);win.document.close();setTimeout(()=>win.focus(),100);
}
