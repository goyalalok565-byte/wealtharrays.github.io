...
function waRenderAccuratePie(id,rows){
 const host=document.getElementById(id+'-graph'); if(!host)return;
 const find=(re)=>rows.find(r=>re.test(String(r.label)));
 let parts=null;
 const investment=find(/^(Total Investment|Total Contributions|Amount Invested|Principal)$/i);
 const gain=find(/^(Interest Earned|Estimated Gain|Net Gain|Profit|Interest)$/i);
 if(investment&&gain&&Number(investment.value)>=0&&Number(gain.value)>=0) parts=[investment,gain];
 else {const regular=find(/^Regular Pay$/i), overtime=find(/^Overtime Pay$/i);if(regular&&overtime)parts=[regular,overtime];}
 if(!parts){host.innerHTML='';return;}
 const total=parts.reduce((a,r)=>a+Number(r.value),0); if(!(total>0)){host.innerHTML='';return;}
 const colors=['#2563eb','#14b8a6']; let start=-90;
