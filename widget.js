...
function waOpenPrintReport(calc,values,results){
 ...
 const html=`<!doctype html><html><head><meta charset="utf-8"><title>${esc(calc.title)} Report</title><style>body{font-family:Arial,sans-serif;max-width:760px;margin:40px auto;padding:0 20px;color:#111}h1{margin-bottom:4px}p{color:#555}table{width:100%;border-collapse:collapse;margin:12px 0 28px}td{padding:10px;border-bottom:1px solid #ddd}td:last-child{text-align:right;font-weight:700}@media print{body{margin:20px auto}}</style></head><body><h1>${esc(calc.title)} Report</h1><p>Generated by Wealth Arrays</p><h2>Your inputs</h2><table>${inputs}</table><h2>Your calculated results</h2><table>${rows}</table><p>Educational estimate only. Not financial, tax, legal or investment advice.</p></body></html>`;
 const blob=new Blob([html],{type:'text/html;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');
 a.href=url;a.download=(calc.slug||calc.id||'wealth-arrays')+'-report.html';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);
}
