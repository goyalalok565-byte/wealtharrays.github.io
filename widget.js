...
async function waCopyEmbed(calc){const src=new URL(`/widget?calc=${encodeURIComponent(calc.id)}`,location.origin).href;const code=`<iframe title="${esc(calc.title)} — Wealth Arrays" src="${src}" width="100%" height="620" loading="lazy" style="border:0;border-radius:16px;max-width:900px"></iframe>`;try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(code);}else{throw new Error('clipboard unavailable')}alert('Embed code copied.')}catch(e){const ta=document.createElement('textarea');ta.value=code;ta.setAttribute('readonly','');ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();ta.setSelectionRange(0,ta.value.length);let ok=false;try{ok=document.execCommand('copy')}catch(_e){}ta.remove();if(ok){alert('Embed code copied.')}else{prompt('Copy this embed code:',code)}}}
